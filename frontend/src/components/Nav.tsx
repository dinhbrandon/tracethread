      ...
      <nav className="relative max-w-[85rem] w-full mx-auto px-4 sm:flex sm:items-center sm:justify-between sm:px-6 lg:px-8" aria-label="Global">
        ...
        <div id="navbar-collapse-with-animation" className={`${isMenuOpen ? '' : 'hidden'} overflow-hidden transition-all duration-300 basis-full grow sm:block`}>
          <div className="flex flex-col gap-y-4 gap-x-0 mt-5 sm:flex-row sm:items-center sm:justify-end sm:gap-y-0 sm:gap-x-7 sm:mt-0 sm:pl-7">
            ...
            {loggedIn ? (
              <>
                <a className="font-medium sm:py-6" href="http://localhost:3000/search">Search</a>
                <a className="font-medium sm:py-6" href="http://localhost:3000/jobnotebook">Job Notebook</a>
              </>
            ) : null}
  
            {/* Dropdown code (unchanged) */}
            <div className="hs-dropdown [--strategy:static] sm:[--strategy:fixed] [--adaptive:none] sm:[--trigger:hover] sm:py-4">
              {/* ... (all your dropdown code here, unchanged) */}
            </div>
